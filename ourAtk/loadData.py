...
class Dataset():
    ...
    def get_adj(self):
        ...
        adj = adj.tolil()
        adj[adj > 1] = 1
        self.lcc_idx = self.largest_connected_components(adj)

        if self.require_lcc:
            print("before choose largest_connected_components",adj.shape)
            lcc = self.largest_connected_components(adj)
            adj = adj[lcc][:, lcc]
            features = features[lcc]
            labels = labels[lcc]
            assert adj.sum(0).A1.min() > 0, "Graph contains singleton nodes"
            print("after choose largest_connected_components",adj.shape)
